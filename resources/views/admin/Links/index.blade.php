...
<div class="crumb_warp">
    <i class="fa fa-home"></i> <a href="{{url('admin/info')}}" target="main">首页</a> &raquo; <a href="{{url('admin/links')}}">友情链接管理</a> &raquo; 友情链接列表
</div>
...
<script>
    function changeOrder(obj,id){
        var orders = $(obj).val();
        $.post("{{url('admin/links/changeorder')}}",{'_token':'{{csrf_token()}}','id':id,'orders':orders},function(data){
            if(data.status == 0){
